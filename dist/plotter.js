...
export class MotionSimulator {
    constructor(canvasId) {
        ...
        this.oldMaxPosition = 0;
        this.oldMaxVelocity = 0;
        this.oldMaxAcceleration = 0;
        this.oldMinPosition = 0;
        this.oldMinVelocity = 0;
        this.oldMinAcceleration = 0;
        this.newMaxPosition = 0;
        this.newMaxVelocity = 0;
        this.newMaxAcceleration = 0;
        this.newMinPosition = 0;
        this.newMinVelocity = 0;
        this.newMinAcceleration = 0;
    ...
    draw() {
        ...
        this.drawPlotWithE(profile, 'vel', eVelocity, maxTime, this.minVelocity, this.maxVelocity, plotHeight, plotHeight, 'green', 'purple', 'Velocity (mm/s)');
        // Draw acceleration plot (bottom third) with both traces
        this.drawPlotWithE(profile, 'acc', eAcceleration, maxTime, this.minAcceleration, this.maxAcceleration, 2 * plotHeight, plotHeight, 'red', 'purple', 'Acceleration (mm/s²)');
    }
    drawPlotWithE(profile, property, eValues, maxTime, minValue, maxValue, yOffset, plotHeight, color1, color2, label) {
        ...
        this.ctx.textAlign = 'right';
        this.ctx.fillText(`Planned extrusion. Max: ${Math.max(...originalValues).toFixed(1)}${suffix}, Min: ${Math.min(...originalValues).toFixed(1)}${suffix}`, width - 50, yOffset + plotHeight - 50 + 10);
        this.ctx.fillText(`With advance Max:. ${Math.max(...adjustedEValues).toFixed(1)}${suffix}, Min: ${Math.min(...adjustedEValues).toFixed(1)}${suffix}`, width - 50, yOffset + plotHeight - 50 + 20);
        this.ctx.textAlign = 'left'; // reset
    }
}
