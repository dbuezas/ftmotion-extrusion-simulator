...
class MotionSimulator {
    constructor(canvasId) {
        this.profile = null;
        this.k = 0.5;
        this.historicMaxPosition = 0;
        this.historicMaxVelocity = 0;
        this.historicMaxAcceleration = 0;
        this.previousAccOvershoot = 1.5;
    ...
    draw() {
        ...
        this.drawPlotWithG(profile, 'acceleration', gAcceleration, maxTime, maxAcceleration, 2 * plotHeight, plotHeight, 'red', 'purple', 'Acceleration (mm/s²)');
    }
    drawPlotWithG(profile, property, gValues, maxTime, maxValue, yOffset, plotHeight, color1, color2, label) {
        ...
        for (let i = 0; i < numTicks; i++) {
            ...
            this.ctx.stroke();
            // Draw label
            this.ctx.fillText(property === 'acceleration'
                ? tickValue.toFixed(1) + 'k'
                : tickValue.toFixed(1), 40, y + 4);
        }
        ...
        const originalValues = profile.map(p => property === 'acceleration'
            ? Math.abs(p[property]) / 1000
            : p[property]);
        const adjustedGValues = gValues.map(v => property === 'acceleration' ? v / 1000 : v);
        const suffix = property === 'acceleration' ? 'k' : '';
        // Display max/min for traces at bottom right
        this.ctx.fillStyle = '#333';
        this.ctx.font = '10px Arial';
        this.ctx.textAlign = 'right';
        this.ctx.fillText(`Max: ${Math.max(...originalValues).toFixed(1)}${suffix}, Min: ${Math.min(...originalValues).toFixed(1)}${suffix}`, width - 50, yOffset + plotHeight / 2 + 10);
        this.ctx.fillText(`w / linear advance Max: ${Math.max(...adjustedGValues).toFixed(1)}${suffix}, Min: ${Math.min(...adjustedGValues).toFixed(1)}${suffix}`, width - 50, yOffset + plotHeight / 2 + 20);
        this.ctx.textAlign = 'left'; // reset
    }
}
